...
const AnthropicLogo = memo(function AnthropicLogo({ className }: { className?: string }) {
  return (
    <svg
      className={className}
      viewBox="0 0 46 32"
...
export const LandingAIInput = memo(function LandingAIInput() {
  ...
  const loadWorkspaceConversations = useCallback(async (workspaceId: string) => {
    if (workspaceConversations[workspaceId] || loadingWorkspaces.has(workspaceId)) return
  ...
  const handleAttachChat = useCallback(
    async (workspaceId: string, conversationId: string, title: string) => {
      ...
      try {
        const existingCompacted = await compactIpc.get(workspaceId, conversationId)
        if (existingCompacted) {
          setAttachedChats((prev) =>
            prev.map((c) =>
              c.id === conversationId
                ? { ...c, status: "ready" as const, content: existingCompacted.compactedContent }
                : c
            )
          )
          return
        ...
        setAttachedChats((prev) =>
          prev.map((c) =>
            c.id === conversationId
              ? { ...c, status: "ready" as const, content: result.compactedChat.compactedContent }
              : c
          )
        )
      } catch {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (attachedChats.length > 0) {
        const readyChats = attachedChats.filter((c) => c.status === "ready" && c.content)
  ...
  }, [input, attachedChats, availableProviders, selectedProvider, selectedModel, router])
      ...
      <div className="rounded-2xl border border-border bg-card/50 backdrop-blur-sm overflow-hidden shadow-lg">
        ...
        <div className="p-4">
          ...
          <textarea
            ...
            className="w-full min-h-[100px] resize-none bg-transparent text-base placeholder:text-muted-foreground/60 focus:outline-none"
            value={input}
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-border/50 bg-muted/20">
                          ...
                          <CollapsibleContent>
                            <div className="ml-6 pl-2 border-l border-border/50 mt-1 space-y-0.5">
                              {loadingWorkspaces.has(project.id) ? (
                                ...
                                (workspaceConversations[project.id] || project.conversations)
                                  .slice(0, 8)
                                  .map((conv) => {
          ...
          <Button
            ...
            disabled={isLoading || anyCompacting || (!input.trim() && attachedChats.length === 0)}
          >
            {isLoading ? (

...
export function AgentsWorkspace() {
  ...
  const loadApiKeys = useCallback(async () => {
    try {
      ...
      pushDebug("error", err instanceof Error ? err.message : "Failed to load API keys")
  ...
  const currentModelId = useMemo(() => {
    if (!bundle?.chat?.modelId) {
      return bundle?.chat?.provider ? getDefaultModelForProvider(bundle.chat.provider as ProviderId) : null
    }
  ...
  const handleMentionSelect = useCallback(async (log: WorkspaceLog) => {
    setMentionOpen(false)
    setMentionQuery("")
    setIsLoadingMention(true)

    try {
      const conversation = await workspaceService.getConversation(log.workspaceId, log.id, log.type)
      if (!conversation) {
      ...
      const wasSummarized = conversation.totalTokenEstimate > MAX_CONTEXT_TOKENS
      let content = formattedContent
      
      if (wasSummarized) {
        content = `[Conversation: "${conversation.title}" - ${conversation.messages.length} messages]\n\n` +
      ...
      pushDebug("info", `Added "${conversation.title}"${wasSummarized ? " (truncated)" : ""} as context`)
    } catch (err) {
  ...
  const handleRemoveMention = useCallback((conversationId: string) => {
    setComposer((prev) => ({
      ...prev,
      mentionedConversations: prev.mentionedConversations.filter((m) => m.id !== conversationId),
    }))
  }, [])
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const shouldGenerateTitle = isFirstMessage && bundle.chat.title === "Untitled chat"
      ...
      <section className="flex flex-1 flex-col overflow-hidden rounded-lg border border-border/40 bg-background/50">
        <header className="flex items-center justify-between border-b border-border/40 px-4 py-2.5">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => fetchChats()} disabled={isLoadingChats}>
              <RefreshCcw className={`h-3.5 w-3.5 ${isLoadingChats ? "animate-spin" : ""}`} />
            </Button>
          </div>
        </header>
          ...
          {!bundle && !isLoadingMessages && (
            <div className="flex flex-1 flex-col items-center justify-center text-center">
              ...
              <p className="text-xs text-muted-foreground/70">
                {chats.length === 0 ? "Create a new chat to begin" : "Choose from the sidebar"}
              </p>
            </div>
          )}
                ...
                {assistantError && (
                  <div className="mb-3 flex items-center justify-between gap-2 rounded-lg border border-destructive/30 bg-destructive/10 px-3 py-2 text-sm text-destructive">
                    ...
                    <Button
                      ...
                      onClick={() => selectedChatId && requestAssistantCompletion(selectedChatId)}
                ...
                <div className="relative flex gap-2">
                  <div className="relative flex-1">
                    ...
                    <textarea
                      ...
                      disabled={composer.isSending || assistantBusy || isLoadingMention}
                    />
                    {isLoadingMention && (
                  ...
                  <Button
                    ...
                    disabled={composer.isSending || !composer.value.trim() || assistantBusy || isLoadingMention}

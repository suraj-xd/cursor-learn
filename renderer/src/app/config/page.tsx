...
import { BackButton } from "@/components/comman/back-button";
  ...
  useEffect(() => {
    const initConfig = async () => {
      ...
      try {
        ...
        const nextPath =
          data.storedPath || data.resolvedPath || data.defaultPath || "";
  ...
  const validateAndSave = async () => {
    try {
      ...
      setStatus({
        type: "error",
        message:
          "Failed to validate path. Please check if the path exists and is accessible.",
      });
    }
  };
      ...
      <div className="space-y-6">
        <div>
          ...
          <Input
            ...
            onChange={(e) =>
              setConfig({ ...config, workspacePath: e.target.value })
            }

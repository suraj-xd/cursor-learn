...
export default function SearchClient() {
  ...
  useEffect(() => {
    const search = async () => {
      ...
      setIsLoading(true)
      try {
        const data = await window.ipc.workspace.search(query, type as 'all' | 'chat' | 'composer')
        setResults(
          (data || []).map(item => ({
            ...item,
            workspaceFolder: item.workspaceFolder || undefined,
          }))
        )
      } catch (error) {
        ...
        {results.map((result, index) => (
          <Card key={index} className="p-4">
            <div className="flex items-start justify-between mb-2">
              <div>
                <Link
                  href={`/workspace?id=${result.workspaceId}&tab=${result.chatId}&type=${result.type}`}
